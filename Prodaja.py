...
def recognize_price(image_price):
    try:
        image = Image.open(image_price)
...
def process_buying():
    ...
    while not stop_event.is_set():
        if step == 0:
            ...
            current_price = recognize_sum('price.png')
            ...
            print("Шаг 2: Определяем цену покупки...")
            screenshot31_path = take_screenshot(screen_price[0], screen_price[1], 
                                              screen_price[2], screen_price[3],
                                              "current_price.png")
            Image.open(screenshot31_path).save('price.png')
            my_price = recognize_sum('price.png')

            screenshot32_path = take_screenshot(screen_enemy[0], screen_enemy[1], 
                                    screen_enemy[2], screen_enemy[3],
                                    "price_of_enemy.png")
            Image.open(screenshot32_path).save('price_of_enemy.png')
            enemy_price = recognize_sum('price_of_enemy.png')
...
class ModernTradeApp:
    ...
    def draw_switch(self):
        ...
        # Закругленные края (рисуем круги в углах)
        corner_radius = 8
        # Левый верхний
        ...
        # Тексты на переключателе (слева и справа)
        left_text = "💰 ПОКУПКА"
        right_text = "🛒 ПРОДАЖА"
        ...
        # Ползунок (рисуем в два слоя для сглаживания)
        if self.mode_var.get() == "buy":
            knob_x = self.padding + 30  # Левая позиция
        else:
            knob_x = self.padding + self.switch_width - 30  # Правая позиция
        
        knob_y = self.switch_height // 2
        
        # Тень ползунка (немного смещена)
        shadow_offset = 2
    ...
    def animate_knob(self, new_mode):
        ...
        if new_mode == "buy":
            target_x = self.padding + 30  # Левая позиция
        else:
            target_x = self.padding + self.switch_width - 30  # Правая позиция
        
        # Находим текущую позицию ползунка
        ...
        for i in range(steps):
            current_x += dx
            # Перерисовываем весь переключатель с новой позицией
            self.draw_switch()
            # Временно обновляем позицию для анимации
            self.root.update()
            time.sleep(0.01)
    ...
    def create_ui(self):
        ...
        # Создаем современный переключатель
        ...
        # Радиокнопки задержки используем обычные
